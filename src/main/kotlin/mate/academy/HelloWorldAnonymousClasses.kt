...
class HelloWorldAnonymousClasses {
    ...
    fun sayHello(names: List<String>): List<String> {
        ...
        val worldGreeting: List<HelloWorldGreeting> = listOf(englishGreeting, frenchGreeting, spanishGreeting)
        val greetings: MutableList<String> = mutableListOf()

        worldGreeting.forEach { greetings.add(it.greet()) }
        names.forEach { name ->
            worldGreeting.forEach { greetings.add(it.greetSomeone(name)) }
        }
        return greetings
    }
}
